...
// @ts-ignore
import { toolPaths } from "../../../meta-ads-mcp-server/tools/paths.js";
...
interface AdToolsProps {
  businessId: string;
  /** Base URL of your running MCP server, e.g. "http://localhost:3001" */
      ...
      for (const path of toolPaths) {
        try {
          const mod = await import(
            `../../../meta-ads-mcp-server/tools/${path}`
          );
            ...
            {connecting ? (
              ...
              <div className="flex flex-col md:flex-row md:space-x-4 space-y-4 md:space-y-0">
                ...
                <div className="flex-1 flex">
                  <input
                    readOnly
                    value={`${mcpServerLink}${sessionPath}`}
                    className="flex-1 border rounded-l-md px-3 py-2 font-mono text-sm"
                  />
                  <button
                    onClick={copyToClipboard(`${mcpServerLink}${sessionPath}`)}
