 ...
 * @param {string} args.campaign_id - The campaign ID to which the ad set belongs.
 ...
 * @param {string} [args.dynamic_creative_optimization_mode="OFF"] - The dynamic creative optimization mode.
 * @returns {Promise<Object>} - The result of the ad set creation.
 */
const executeFunction = async ({ account_id, campaign_id, token, name = "PostMancAdsetTrafficCampaignTargetIG", billing_event = "LINK_CLICKS", targeting = { "age_max": 25, "age_min": 22, "geo_locations": { "countries": ["NP"], "location_types": ["home"] }, "genders": [1], "publisher_platforms": ["instagram"] }, start_time = "2024-06-16T04:45:17+0000", status = "PAUSED", dynamic_creative_optimization_mode = "OFF" }) => {
  const baseUrl = 'https://graph.facebook.com/v12.0';
  const url = `${baseUrl}/act_${account_id}/adsets?campaign_id=${campaign_id}&name=${encodeURIComponent(name)}&billing_event=${billing_event}&targeting=${encodeURIComponent(JSON.stringify(targeting))}&start_time=${start_time}&status=${status}&dynamic_creative_optimization_mode=${dynamic_creative_optimization_mode}`;
    ...
    function: {
      ...
      parameters: {
        ...
        properties: {
          ...
          campaign_id: {
            type: 'string',
            description: 'The campaign ID to which the ad set belongs.'
          },
          token: {
            type: 'string',
          ...
          dynamic_creative_optimization_mode: {
            type: 'string',
            description: 'The dynamic creative optimization mode.'
          }
        },
